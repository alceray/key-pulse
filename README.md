...
Then open Settings, choose PostgreSQL, and fill in the host, port, database, user, and password. Test confirms both that the role can sign in and that it can create the schema. **Save and restart** saves the connection and pending switch, then restarts KeyPulse automatically after replacement is confirmed. The old process finishes flushing input and releasing database locks before the new process starts the transfer. The window reopens after the restart, including in tray mode. PostgreSQL connection edits use the same save-and-restart action.

During a transfer, a progress window shows preparation, copying, verification, and activation. **Stop and exit** requests cancellation and waits for database work to finish before closing. Unfinished changes remain pending for the next launch. The completion log records source, destination, elapsed time for the current attempt, and whether it resumed an already committed copy.

To move directly between PostgreSQL databases, choose **Edit** under Storage, enter the destination and its password, then choose **Save and restart**. Confirm the source and destination names before replacing destination history. Both databases are locked during the transfer, and the source history is retained. The active connection and password stay available until the verified destination is activated. Changing only the current database's username, password, or SSL mode updates authentication without replacing history. The saved password is prefilled and masked. Test and Save use the password shown in the form, so update it if the destination needs a different password. Clearing it requires entering a password again. Hover over the info icon beside Storage for switching help.

Only one database change can be pending. Finish it by restarting or cancel it before choosing another destination. Recovery identifies the affected connection and lets you correct **Source** or **Destination** authentication while keeping the confirmed endpoints fixed. A matching destination completion marker finishes activation even if the source is unavailable. Canceling an unfinished change keeps the source connection and password.
